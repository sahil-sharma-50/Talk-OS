...
TalkOS is an AssemblyAI-powered productivity workspace agent. Speak or type an outcome and it turns the conversation into coordinated documents, working spreadsheets, plans, and source-backed research. Interrupt mid-task to change a constraint; active work is cancelled, stale results are discarded, and related files can be updated as one undoable change.
...
- **Planner:** create tasks, set due/start/end times, flag overlaps, and export task CSV or calendar ICS.
- **Research:** keep every Tavily search grouped with its results, extracted source text, and sourced summary.
- **Coordination:** apply related document, sheet, and planner edits atomically with revision checks and a single Activity receipt.
