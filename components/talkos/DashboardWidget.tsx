...
export function DashboardWidget({ widget, state, onOpenSource, onRemove }: { widget: DashboardWidgetDefinition; state: WidgetState; onOpenSource: (source: DashboardSource) => void; onRemove: () => void }) {
  ...
  return <article className="dashboard-widget" data-size={widget.size} data-kind={widget.binding.kind} style={widget.color ? { "--widget-accent": widget.color } as React.CSSProperties : undefined}>
    <header><div><h3>{widget.title}</h3></div><button type="button" onClick={onRemove} aria-label={`Remove ${widget.title}`}>×</button></header>
