...
export function DashboardWorkspace({ workspace, onChange, onOpenSource }: { workspace: WorkspaceSnapshot; onChange: (workspace: WorkspaceSnapshot) => void; onOpenSource: (source: DashboardSource) => void }) {
  ...
  if (!active) return <div className="tool-empty"><BarChart3 size={30} /><h2>Bring the project into focus</h2><p>Create a dashboard, select the Documents, Sheets, Planners, and Research for this project, then add linked widgets. They update automatically when source data changes, and TalkOS can build or edit the view by voice.</p><button type="button" onClick={create}><FilePlus2 size={14} /> Create a dashboard</button></div>;
  ...
  return <div className="dashboard-workspace">
    ...
    <section className="dashboard-main">
      ...
      <aside className="dashboard-guide" role="note" aria-label="How Dashboard works"><strong>How Dashboard works</strong><p>Select the Documents, Sheets, Planners, and Research that belong to this project, then add the metrics, charts, and summaries you need. Every widget stays linked to those sources and updates automatically when their data changes. You can also ask TalkOS by voice to build or change this dashboard.</p></aside>
